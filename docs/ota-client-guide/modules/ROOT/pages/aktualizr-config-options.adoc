...
| `update_lock_file`              | `/run/lock/aktualizr-lock` | If this file exists, and a flock() lock is held then updates will be disabled.
|==========================================================================================

The default for `update_lock_file` depends on the setting of the `TORIZON` build flag.
When not building in TORIZON mode `update_lock_file` defaults to ``.
...
| `rollback_command`     | `"fw_setenv rollback 1"`        | Command that arms a bootloader rollback of the OS after a sync group fails. The device is rebooted only if this command succeeds.
|==========================================================================================
