...
PackageManagerInterface::FetchResult PackageManagerInterface::fetchTarget(const Uptane::Target& target,
  ...
  try {
    ...
    if (!response.isOk()) {
      ...
      throw Uptane::Exception("image", "Could not download file from " + target_url + ": " + response.getStatusStr());
    }
    if (!target.MatchHash(Hash(ds->hash_type, ds->hasher().getHexDigest()))) {
      ...
      // computed hash, the expected hash(es), how many bytes were downloaded, and
      // the HTTP status -- so the failure reported to the server is diagnosable.
      ...
      throw Uptane::Exception("image", "Hash mismatch for target " + target.filename() + " downloaded from " +
                                           target_url + ": expected " + Hash::encodeVector(target.hashes()) +
                                           ...
                                           " expected bytes; HTTP status: " + response.getStatusStr());
    }
    ds->fhandle.close();
    return {true, ""};
