...
void check_repo(const TemporaryDirectory &temp_dir, const Uptane::RepositoryType repo_type) {
  ...
  const auto snapshot = Uptane::Snapshot(repo_type, Uptane::Role::Snapshot(), Utils::parseJSON(snapshot_raw),
...
TEST(uptane_generator, sign) {
  ...
  std::string sign_cmd = R"(echo "{\"_type\":\"Snapshot\",\"expires\":\"2029-07-04T16:33:27Z\"}" | )" + cmd;
  output.clear();
